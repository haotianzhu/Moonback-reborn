...
userRouter.patch('/:id', (req, res) => {
    ...
    if (hasPermission) {
        User.generateHashPassword(reqUser.password, async (error, newHashPassword) => {
            // get new information
            if (error) {
                console.log("PATCH api/user/:id => 520", error)
            ...
            await User.findByIdAndUpdate(authUserid, newUser, { useFindAndModify: false }, (error) => {
                ...
                console.log("PATCH api/user/:id => 200");
            });
        });
    } else {
        // no permission
        console.log("PATCH api/user/:id => 403 ","no permission");
